CREATE FUNCTION funct_load_cliente_divida(filter jsonb)
...
begin
  ...
  return query select
      ...
      sum( cp.compra_montantepagar ) filter ( where cp.compra_tcompra_id = _const.tipocompra_compra ) as cliente_montentecompras,
      sum( cp.compra_montantepagar ) filter ( where cp.compra_tcompra_id = _const.tipocompra_divida ) as cliente_montentedividas,
      sum( cp.compra_montantepagar ) as cliente_montantetatal,
      sum( cp.compra_montanteamortizado ) as cliente_montanteamortizado,
      sum( cp.compra_montantepagar - cp.compra_montanteamortizado ) as cliente_montantependente
    from cliente cli
      ...
      left join ggviario.venda cp on cli.cliente_id = cp.compra_cliente_id
